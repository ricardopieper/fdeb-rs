...
impl Fdeb {
    ...
    fn _compute_divided_edge_length_approx(&self, subdivision_points_for_edge: &[Vertex2D]) -> Float {
    ...
    fn project_point_on_line(
        ...
        let l = (q_target.x - q_source.x).powi(2)
            + (q_target.y - q_source.y).powi(2);
    ...
    fn compute_forces_on_point(
        ...
        let mut resulting_force = Vertex2D { x: 0.0, y: 0.0 };
    ...
    fn update_edge_divisions(
        ...
        assert_eq!(subdivision_points_for_edge.len(), self.edges.len());
        ...
        let deferred_set: Vec<_> = edge_subdivs
            .map(|(subdivisions, edge)| {
                if p == 1 {
                    let mut new_subdivision_points = Vec::<Vertex2D>::with_capacity(3);
                    new_subdivision_points.push(self.vertices[edge.source].clone()); // source
                    new_subdivision_points.push(self.edge_midpoint(edge)); // mid point
                    new_subdivision_points.push(self.vertices[edge.target].clone()); // target
                    ...
                    let mut new_subdivision_points = Vec::<Vertex2D>::with_capacity(subdivisions.len() + 2);
        ...
        for _ in 0..C {
            for _ in 0..i {
                for edge in 0..self.edges.len() {
                    let points = self.apply_resulting_forces_on_subdivision_points(
                        edge,
                        p,
                        s,
                        &edge_subdivisions,
                    ...
                    for i in 0..=p {
                        edge_subdivisions[edge][i].x += points[i].x;
                        edge_subdivisions[edge][i].y += points[i].y;
                    }
                }
            }
